...
dicts = {}
with open("./save/labels.txt", 'r', encoding="utf-8") as f:
...
class Infer(object):
    ...
    """
    def __init__(self):
        self.vocab_processor = learn.preprocessing.VocabularyProcessor.restore('./save/vocab.pickle')
        self.checkpoint_file = tf.train.latest_checkpoint('./save')
        graph = tf.Graph()
...
if __name__ == '__main__':
    info=Infer()
    prediction,score=info.infer(['#重庆身边事#重庆万州大巴车坠江事件，不造谣不传谣。'])
